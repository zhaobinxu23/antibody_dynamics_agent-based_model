...
L = Matrix.interaction_M/5; %% modeling the lockdown effect to 20% of its original contact matrix.


delta_t = 1; %% represent 0.1day     time unit = 0.1day
virus_complex  = zeros(1000,14000);%% represent 1000 people over 1400 days
...
for i = 1:140
    ...
    for j = 1:1000
        ...
        virus(j,(kk-1)*140+i+1) = max((-varied_para_1(j) *antibody(j,(kk-1)*140+i)*virus(j,(kk-1)*140+i) + para(2)*virus_complex(j,(kk-1)*140+i) - para(6)*virus(j,(kk-1)*140+i) + (para(7)*0.99995^((kk-1)*140+i))*virus(j,(kk-1)*140+i) + dd)*delta_t + virus(j,(kk-1)*140+i),0);
